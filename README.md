...
| Code languages        | Required. A fence without one is an unfixable error: both subcommands report it and exit 1, and the block is left exactly as written. An indented code block can never carry a language, so it is always this error; convert it to a fence. |
